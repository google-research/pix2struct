...
def render_text(text: str) -> Image.Image:
  ...
  wrapped_text = "\n".join(lines)
  font = ImageFont.truetype(FONT, encoding="UTF-8", size=12)

  # Use a temporary canvas to determine the width and height in pixels when
  # rendering the text.
  temp_draw = ImageDraw.Draw(Image.new("RGB", (1, 1), "white"))
  _, _, width, height = temp_draw.textbbox((0, 0), wrapped_text, font)

  # Create the actual image with a bit of padding around the text.
  image = Image.new("RGB", (width + 10, height + 10), "white")
  draw = ImageDraw.Draw(image)
  draw.text(xy=(5, 5), text=wrapped_text, fill="black", font=font)
  return image
...
def render_text_on_bounding_box(
  ...
  def _can_increment_font(ratio=0.95):
    next_font = ImageFont.truetype(FONT, encoding="UTF-8", size=fontsize + 1)
    width, height = next_font.getsize(text)
    return width < ratio * (x1 - x0) and height < ratio * (y1 - y0)

  while _can_increment_font():
    fontsize += 1
  font = ImageFont.truetype(FONT, encoding="UTF-8", size=fontsize)
